...
async function getRepo({ config, org, project, repo }) {
  ...
  // A repo GUID is unique across the org, so it resolves without a project. That
  // matters: the caller looking up a GUID (e.g. from a work item's artifact link)
  // does not yet know which project owns it — that is the thing being asked.
  const looksLikeId = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(repo));
  const scope = (project && !looksLikeId) ? `/${encodeURIComponent(project)}` : '';
  return request(`${base}${scope}/_apis/git/repositories/${rep}?api-version=${API}`, { pat: config.pat });
}
