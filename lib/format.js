...
// Reuse the single vote scale defined alongside the PR API (pr.js requires only
// ./api, so there is no cycle).
const { voteLabel } = require('./pr');
...
const { stripHtml } = (() => {
  function stripHtml(html = '') {
    return html
      .replace(/<br\s*\/?>/gi, '\n')
      .replace(/<\/p>/gi, '\n')
      // Table cells need a visible separator, otherwise a row collapses into one
      // run-together string ("#ScenarioExpected") when read back in the terminal.
...
function printPullRequest(pr, extras = {}) {
  const lines = [
  ...
  // "Did it ship, and as which commit?" Both live in the raw payload and were
  // missing from the human output, so the answer came from re-reading --json.
  //
  // lastMergeCommit is NOT proof of shipping: on an open PR it is Azure's trial
  // merge — a commit that exists, looks exactly like a real one, and has not been
  // merged anywhere. Only label it a merge commit once the PR is completed.
  const completed = String(pr.status).toLowerCase() === 'completed';
  if (pr.closedDate) lines.push(`Closed:    ${new Date(pr.closedDate).toLocaleString()}`);
  const merge = pr.lastMergeCommit && pr.lastMergeCommit.commitId;
  if (completed && merge) {
    lines.push(`Merge commit: ${merge}`);
  } else if (completed) {
    lines.push('Merge commit: (none reported — squash/rebase completion or a stale payload)');
  } else if (merge) {
    lines.push(`Merge preview: ${merge}  (trial merge — NOT shipped)`);
  }
  lines.push(`URL:       ${pr.url || '—'}`);

  // Linked work items, when the caller fetched them. An explicit "(none)" matters:
  // a PR with no work item is a finding, not a missing field.
  if (extras.workItems) {
    const ids = extras.workItems.map((w) => `#${w.id}`);
    lines.push(`Work items: ${ids.length ? ids.join(', ') : '(none)'}`);
  }

  // --full: reviewer votes and thread counts.
  if (extras.threads) {
    const reviewers = pr.reviewers || [];
    if (reviewers.length) {
      lines.push('Reviewers:');
      for (const r of reviewers) {
        lines.push(`  ${r.displayName || '?'} — ${voteLabel(r.vote)}${r.isRequired ? ' (required)' : ''}`);
      }
    } else {
      lines.push('Reviewers: (none)');
    }
    const active = extras.threads.filter((t) => !t.isDeleted);
    const open = active.filter((t) => t.status === 'active' || t.status === 'pending');
    const anchored = active.filter((t) => t.threadContext?.filePath);
    lines.push(`Threads:   ${active.length} (${open.length} open, ${anchored.length} anchored to a file)`);
  }
