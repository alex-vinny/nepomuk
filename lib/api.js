...
function attempt(rawUrl, { method = 'GET', pat, body, headers = {} } = {}) {
  return new Promise((resolve, reject) => {
    ...
    // A binary body is sent as-is — that is how a file upload (attachments) works.
    // Anything else is JSON. The test is ArrayBuffer.isView, not Buffer.isBuffer:
    // a Uint8Array (what arrayBuffer(), crypto and worker messages hand you) is NOT
    // a Buffer, and JSON.stringify would turn it into {"0":37,"1":80,...} — which
    // Azure accepts with a 200 and stores as a corrupt attachment. Buffer itself is
    // a Uint8Array, so the one test covers both.
    const payload = body
