...
//
// Why this is a module and not a template someone retypes: a hand-built link went
// out with the wrong project segment and had to be edited after publication. The
// project is NOT guessable from the board you happen to be looking at — several
// EVUP repos live outside `ELOS`, and work items are split across `EVUP` (the old
// board) and `Kanban EL` (the current one). The authoritative source is the ROOT
// SEGMENT OF THE WORK ITEM'S AREA PATH, which is always the project name.
//
 ...
 * `Kanban EL\Digital\Squad 4` -> `Kanban EL`. Accepts either slash.
...
function azureUrl({ org, project, kind, repo, id }) {
  ...
  if (!project) throw new Error('azureUrl: project is required — resolve it from the Area path root, do not assume ELOS.');
...
/**
 * The reference line that closes a review, per CONVENTIONS.md:
 *
 ...
 * when you paste a link, and it is the shape Alex recognizes. `[<Repo>]` is the
 * real repository taken from the PR, not the service name that sounds like it
 * matches the subject: the PR title alone never says where the change lives.
 *
...
/**
 * Work-item counterpart. CONVENTIONS.md specifies only the PR line, so this
 * mirrors its shape: the work item type takes the place of the repo tag.
 */
